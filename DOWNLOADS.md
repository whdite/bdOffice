...
```sh
minisign -Vm BigDogOffice-0.8.7-windows-x64.exe -P RWQzJu6+Jww+ZBxyLybA+HIpfIIp+DBGe0/dA29WMOaZW2F/w6UPTxYC
```
...
- 固定版本资源：`https://github.com/whdite/bdOffice/releases/download/v0.8.7/<资产文件名>`。
...
当前 0.8.7 是正常版本递增，0.8.6 用户可以收到新版本提示。此前 0.8.6 同版本重发的说明保留在历史日志中。缓存节点如出现校验不符，应重新获取最新清单/资产，不能忽略错误继续安装。
